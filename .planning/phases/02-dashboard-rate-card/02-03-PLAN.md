...
<objective>
Build the Rate Card Explorer page — a searchable, sortable table of labor roles with rate ranges, GL codes, and expandable detail rows.

Purpose: Deliver clean reference documentation of DriveShop's 30 labor roles with rate intelligence. This is the simplest of the three views — basic-but-clean.
Output: Searchable/sortable rate card table with expandable rows showing unit/cost rate breakdowns.
</objective>
...
<task type="auto">
  ...
  <action>
Replace the RateCardPage stub with the full Rate Card Explorer. Use `getRateCard()` data accessor.

**Rate Card Explorer content:**

1. **Header section**: "Rate Card Explorer" title with description "30 labor roles extracted from 1,659 historical event estimates. Showing unit and cost rate ranges, GL codes, and usage frequency." Use Card wrapper with CardHeader.
...
Keep all code in RateCardPage.tsx. No separate component files.
  ...
  <verify>npm run build succeeds. Dev server shows Rate Card Explorer with search input, 3 summary cards, sortable table with 30 roles, and expandable detail rows.</verify>
  <done>Rate Card Explorer shows: search input filtering roles, 3 summary stat cards, sortable 30-row table with GL code badges and OT indicators, clickable column headers with sort direction indicators, expandable detail rows with unit/cost rate ranges and visual range bars. All data from getRateCard().</done>
</task>
...
<task type="auto">
  ...
  <files>src/pages/RateCardPage.tsx</files>
  <action>
Refine the Rate Card Explorer with these presentation details:
...
7. **Overall layout consistency**: Ensure the Rate Card page visual weight matches the Dashboard page — same Card padding, same header spacing, same muted-foreground text color for descriptions.
  ...
  <done>Rate Card Explorer polished: occurrence frequency bars visible, rate ranges consistently formatted, empty search state shown, rows keyboard-accessible, visual consistency with dashboard. Phase 2 complete.</done>
</task>
...
- Rate Card Explorer fully functional with search, sort, expand
- Visual quality matches enterprise aesthetic from Dashboard
- Phase 2 complete: Dashboard (4 tabs) + Rate Card Explorer all functional
  </success_criteria>

<output>
After completion, create `.planning/phases/02-dashboard-rate-card/02-03-SUMMARY.md`:

# Phase 2 Plan 03: Rate Card Explorer Summary
