...
| 022 | Client Accounting Identity + VW Merge | Accounting go-live | planned | Loads the 21 Intacct customer IDs Tatiana confirmed, collapses the duplicate VW/Volkswagen client into one record without losing its 71 prices or its 80% payout, and produces a read-only audit stating exactly which export-required fields are still empty. No application code. |
