...
- **HANDED OVER 2026-08-27: production is DriveShop's Render, deploying `main` of `DriveShopDave/Event_Estimate`.** Push to `origin` (= DriveShopDave) to deploy; `mine` (= aininja-pro/Event_Estimate) is Ray's backup mirror. Supabase ownership transferred the same day (project ref/URL/keys unchanged by transfer). AI runs on DriveShop's Anthropic key with model `claude-opus-5` — a new org cannot use retired snapshots, so never pin a dated model id again. Render binds one GitHub identity per Render user; DriveShop's Render uses the DriveShopDave identity so it never contends with Ray's. Everything below this line describes the pre-handover history.
- **Deploy branch is `main` (since 2026-08-10, Sprint 021).** Both Render services (`event-history` static + `driveshop-api`) build and deploy from `main`, which auto-syncs on every push to it. Push to `main` to deploy. Operator changed the Branch setting on each service; confirmed by Ray 2026-08-10.
  - **History, so old notes read correctly:** from the first deploy (2026-07-02) until 2026-08-10 the tracked branch was **`sprint-018-office-cost-correction`**, and `main` was behind. Any planning note written before 2026-08-10 that says production runs off a feature branch is describing that period, not the current setup.
  - **The consolidation was a fast-forward, not a merge.** At the time of the switch, `origin/sprint-018-office-cost-correction` contained **zero** commits absent from `main`, and `main` was exactly **one** commit ahead: `8800ca1` (Sprint 020). So repointing deployed precisely the Sprint 020 price guard and nothing else. Verified with `git rev-list --count` in both directions before the change.
  - **Consequence for Sprint 020:** the price data went live on 2026-08-02 when the operator applied the SQL directly to Supabase, but the guard code (`isUnpricedRate`, the `estimate → in_review` gate, pickers, Summary banner) only reached production with this branch switch. Between 2026-08-02 and 2026-08-10, production had real prices and no guard.
...
## Client Accounting Identity (Sprint 022)

- **The client-to-Intacct-customer mapping is a confirmed literal table, not a derivation.** Tatiana supplied `Customers 8.10.26.xlsx` (296 customers) on 2026-08-10 and confirmed every ambiguous case by email the same day. The table lives in `scripts/import_client_accounting.py` as `CUSTOMER_IDS` and in `planning/QUESTIONS.md` with full provenance. **Never re-derive it by name or GL matching** — Sprint 019 proved inference produces wrong Intacct identity.
- **Two customer IDs are shared by two client records each, deliberately.** `C0004` covers **Acura and Honda** (DriveShop bills them through one "Acura/Honda" account) and `C0099` covers **Volvo and Volvo MS** (Volvo MS is a program under Volvo running its own rate table). **Any uniqueness validation on `intacct_customer_id` must permit duplicates.** A "fix" that dedupes these is a regression.
- **`Lexus` maps to `C0134` "Team One Lexus", an agency, not Lexus.** Confirmed by Tatiana: Lexus work bills through the agency and there is no direct Lexus account. It looks wrong at a glance; it is correct.
- **VW and Volkswagen merged into one client named `VW` on `C0208`.** The trap, and why the sequence matters: Dave's pricing tab was named "Volkswagen", so all **70** priced rate-card rows loaded onto the `Volkswagen` record, while the `VW` record was empty **but carried the non-standard `office_payout_pct` of 0.8000** (every other client is 0.7500). So the record to keep by data and the record to keep by name were different records, and the valuable setting was on the one being deleted. The merge therefore ran: carry the payout across → delete the empty `VW` (freeing the UNIQUE `name` **and** `code`) → rename `Volkswagen` to `VW` and take code `VW` + customer `C0208`. Renaming first collides on both unique columns.
- **`rate_card_items.client_id` is `ON DELETE CASCADE`** (confirmed against the live catalog via `information_schema`, not just the migration files; `client_contacts` is CASCADE too, `estimates` is NO ACTION). **Deleting a client silently destroys its entire rate card with no error.** Any future work that deletes or merges a client must guard the delete explicitly. `scripts/import_client_accounting.sql` does this with a pre-delete assertion and a pre-commit verification, both inside one transaction.
- **`default_currency` = `USD` and `default_exchange_rate_type` = `Intacct Daily Rate` were already populated on every client**, so those two export fields were never actually blockers despite being listed as unknowns.
- **Still missing for a valid export, and not fixable by us:** `default_payment_terms` (AR, no fallback — only Mazda has it, "Net 30") and `default_department_id` (no fallback anywhere; office profiles carry a location but no department). Both were blank on the Client Settings tab Dave returned. Open as QUESTIONS #14 and #15 for Tatiana.

## Export Identity: Department + Payment Terms (Sprint 023)

- **The Intacct Department IS the revenue segment, and it belongs to the EVENT, not the client.** Tatiana, 2026-08-10: *"the Department refers to the Revenue Segment ... The Department is associated with the event, not the client."* Her department list matches the 10 `revenue_segments` loaded in Sprint 019 exactly, codes and names, so `revenue_segments.code` **is** the Department ID. No data load was needed; the exporter simply was not reading it.
- **Department fallback order, and it must not be reordered:**
  ...
  The revenue segment is **last on purpose**. Putting it earlier silently overrides an explicit department someone set deliberately, and the export would still produce a plausible-looking value, so the bug would not announce itself. `client.default_department_id` stays in the chain even though department is event-level: the column exists and removing it is an unrequested behavior change.
- **AR payment terms are per-client free text** (`"Net 30"`, `"Net 45"`, `"Net 60"`), taken from Intacct's own customer list. Unlike AP, AR has **no fallback** — a null blocks every invoice for that client. If a real export turns out to need a term **ID** rather than the label, that is a Sprint 025 finding; do not guess at a code.
- **Office payout stays PER CLIENT. Do not build per-item payout.** (Ray, 2026-08-10, closing QUESTIONS #6.) DriveShop's catalog does set payout per item (100% pass-throughs, 50% planning and admin, 90% chauffeurs, 75% everything else) and the app applies one figure per client (0.75, and 0.80 for VW). Reasoning for keeping it: switching to per-item rewrites office-event margin calculation across the app, and that formula was found **inverted** and corrected only recently in Sprint 018 Phase 1. Reopening it is not worth the risk without evidence the variance is material. **Revisit only if Dave reports that a meaningful share of office labor falls outside the standard 75%.**
- **Estimate-facing item list, proposed rather than asked.** Only **83 of the 160** catalog items appear on any client's rate card; **77** have never been priced by anyone. Those 83 are the natural answer to Dave's blank "In event estimate?" column. Propose the 83 for confirmation instead of asking him to fill in 160 rows.

## Catalog Amendments — decided without Dave (Sprint 024)

Dave did not reply and waiting was blocking the beta. Ray authorised best-guess decisions on 2026-08-10. All are reversible.

- **Volvo MS rate card seeded from Volvo (69 rows).** Tatiana confirmed Volvo MS is a program under the Volvo client with its own rate table, but Dave sent no Volvo MS tab, so it had zero prices: invoiceable and unable to produce an estimate. Volvo is the closest defensible source (same Intacct customer `C0099`, same 20% trucking markup, same 2.5% third-party markup). The copy is idempotent — a re-run inserts nothing — so rates edited later are never overwritten.
- **`I0124` Right Seat Driver - Travel gets an $80 overtime rate on Lucid and MB.** Dave priced the standalone Travel O/T item at exactly 80 on both tabs but left the parent row's overtime column blank; Sprint 020 correctly declined to invent it. Intent is unambiguous, and the alternative is those two clients being unable to bill Travel overtime at all.
- **Three items were NOT guessed, on principle.** They need an Intacct **Item ID**, which only DriveShop accounting can issue. Guessing does not produce a wrong price somebody notices — it produces a **fake identifier inside their accounting system, on real invoices, reconciling against nothing**. That is the line: guess freely on business logic, never invent an identifier that lands in someone else's ledger.
  - **Chauffeur "High Markets"** (`I0217` duplicate). Maserati is unaffected (both rates were 100). **Volvo bills chauffeur hours at 90 instead of 100** until a second Item ID exists. This is a known, quantified revenue gap, not an unknown.
  - **EV Charging - Flat Rate** and **Per Diem - Flat Rate** stay as the existing pass-throughs (`I00601`, `I0090`), billing on receipts at the client markup.
  - **`I00601`'s five-digit Item ID** stays as DriveShop wrote it. "Correcting" it to `I0060` could collide with a real ID.
- **Three of Dave's questions needed no work at all**, verified rather than assumed:
  - The four **"Duplicate???"** items (`I0202`, `I0203`, `I0204`, `I0205`) are on **zero** rate cards — he left them unpriced on all 20 tabs and the Sprint 020 loader skips blanks. Already invisible in the estimate builder.
  - The blank **"In event estimate?"** column is **moot**: the builder reads each client's `rate_card_items`, never the full `fee_types` catalog, so the 77 items nobody priced already never appear.
  - **GL `4000.99`** is referenced nowhere in the codebase or any load script.

## Intacct Export Scalars (Sprint 025)

- **AP bill `dueDate` is derived from payment terms.** Only the AP template has a `dueDate` column; the AR invoice template sends `paymentTerms` and lets Intacct derive the date, which is why there is no AR equivalent. `dueDateFromTerms()` in `accounting-csv-service.ts` parses the day count out of free text ("Net 30" → +30 days) and adds it to the bill date in **UTC**. Local-time date maths would shift the result by a day either side of midnight depending on where the exporter runs. **Unparseable terms return blank, never a guessed date** — a blank due date is a visible gap an accountant fills; a wrong one posts silently. An explicit `options.dueDate` or line-level value still wins.
- **Default scalars confirmed, no change needed.** `transactionType` defaults to `'Sales Invoice'` and `exchRateType` to the client's `default_exchange_rate_type`, falling back to `'Intacct Daily Rate'`. Every client already carries `USD` / `Intacct Daily Rate`, so these were never actually blockers despite being listed as open since Sprint 018.
- **Intacct export stays office-only.** `canExportIntacct()` returns false for any non-office `cost_structure`. This is existing, deliberate design, not an oversight — corporate events are not invoiced through this path. Corporate-event export scope remains closed; revisit only if DriveShop asks for it. Related: corporate estimates record no cost at all (Sprint 026), so exporting them today would produce ~100% margin bills.

## Corporate Event Cost (Sprint 026)

- **Corporate cost = 50% of the unit rate; pass-throughs = 100%.** Confirmed by Ray 2026-08-10. Recovered from the Sprint 016 cost rate card (still in git at `scripts/import_rate_cards.sql`), which was real DriveShop cost data and strikingly consistent: 0.50 in 66/66 Planning & Admin rows, 142/163 Onsite Labor, 11/11 Creative, 249/392 Logistics. The exceptions all sit at 1.00 and are pass-throughs — DriveShop passes those at cost and earns only the client markup.
- **Stored as a PERCENT, not a flat dollar amount** (`corporate_cost_is_percent = true`), so a later rate change carries the cost with it instead of leaving a stale figure behind.
- **The old rows were NOT copied item by item.** Sprint 016 used pre-Sprint-019 item names ("Event Director Day (10 hr)") and the current catalog uses DriveShop's ("Event Director Days"). Matching them is exactly the reconciliation Sprint 019 proved impossible. The flat rule sidesteps it and reproduces ~85% of the old values exactly.
- **`office_cost` deliberately stays NULL, and a guard enforces it.** Office cost is *derived* at estimate time from `clients.office_payout_pct` (the W8 fix). Writing a copy onto 1,397 rate-card rows would create 1,397 places to drift out of sync with the client record. One source of truth per structure: office from the client, corporate from the rate card item.
- **`corporateCostRate()` in `estimate-totals.ts` is the single implementation**, mirroring `officeCostRate()`. Called by both add paths (`EstimateBuilderPage`, `ScheduleGrid`) **and both halves of the Corporate↔Office toggle recompute** (labor entries and schedule entries). W8 lived in five sites and was wrong in all of them; do not inline this formula anywhere.
- **It returns `null`, never `0`, when a row has no corporate cost** — a custom role, or an item nobody costed. Null means "not known" and the UI keeps the cell editable. Zero would read as 100% margin and be indistinguishable from a real answer. `ScheduleGrid` coerces to 0 only because `schedule_entries.cost_rate` is non-null in the schema.
- **The toggle recompute looks corporate cost up by `rate_card_item_id`** against the loaded `rateCardData`, because a `labor_entries` row does not carry the cost itself. Rows with no rate card item resolve to null and stay user-editable, unchanged from before.
