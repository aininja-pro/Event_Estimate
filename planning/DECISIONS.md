...
- **Office-event labor cost/GP formula is INVERTED** (deferred, Sprint 017 W8 — investigation complete, fix pending Tatiana's confirmation) — For office-structure estimates, labor `cost_rate` is computed at row-add time as `day_rate × (1 − office_payout_pct)` in `ScheduleGrid.tsx` (`handleAddSelected`, lines ~148/159) and `EstimateBuilderPage.tsx` (~1076/1087). DOMAIN says the office **receives** `office_payout_pct` of revenue (≈75%), so that share is DriveShop's **cost**; cost should be `day_rate × payout`, GP = `day_rate × (1 − payout)`. The current formula is backwards (it books the office's 75% share as GP and DriveShop's 25% as cost).
  - **Three-signal agreement that the code is inverted:**
  ...
  - **Two linked bugs — fix as a PAIR, under full gate, once Tatiana confirms direction:**
    - **Issue ① (recompute on toggle):** `cost_rate` is frozen at row-add time; toggling `cost_structure` (or changing `office_payout_pct`) afterward does not re-derive existing rows. Make office cost a derived value (recompute on change, or compute in the rollup/derivation rather than storing at add-time).
    - **Issue ③ (formula direction):** invert to `cost = day_rate × office_payout_pct`, GP = `day_rate × (1 − office_payout_pct)`.
  - **Operational guardrail until confirmed and fixed: office-event cost and GP figures are inverted and unreliable** — do not rely on office-structure GP/cost in estimates, PDFs, snapshots, or thresholds until this is reconciled. (Corporate-structure estimates are unaffected.)
